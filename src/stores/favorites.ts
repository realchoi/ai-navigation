...
export const useFavoriteStore = defineStore("favorites", () => {
  ...
  const favorites = ref<AiWebsiteDto[]>(
    JSON.parse(localStorage.getItem("aiFavorites") || "[]")
  );

  const isFavorite = (id: string) =>
    favorites.value.some((item) => String(item.id) === id);

  const addFavorite = (site: AiWebsiteDto) => {
